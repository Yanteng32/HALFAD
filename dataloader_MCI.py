...
class DataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dir == 'PMCI/':
            label = 1  # Progressive Mild Cognitive Impairment
        elif self.dir == 'SMCI/':
            label = 0  # Stable Mild Cognitive Impairment
        elif self.dir == 'MCI/':
            label = 2  # Mild Cognitive Impairment (general)
...
def load_data(args, root_path, path1, path2):
    ...
    :param root_path: Root directory containing data
    :param path1: Path to first class data (e.g., PMCI)
    :param path2: Path to second class data (e.g., SMCI)
    :return: DataLoader for combined dataset
    """
    # Create datasets for both classes
    train_PMCI = DataSet(root_path, path1)
    train_SMCI = DataSet(root_path, path2)
